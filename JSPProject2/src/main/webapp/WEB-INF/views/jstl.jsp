<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8"%>
 ...
 <body>
    ...
    <ol>
        ...
        <pre>
            JSP 파일 최상단에 취가하고자 하는 JSTL taglib를 추가
        </pre>
        ...
        <h4> ${ num2 }</h4> <!-- 20. 왜? 우선순위가 달라서(범위가 좁은게 우선순위 높음) -->
		<h4> ${ requestScope.num2 }</h4> <!-- 300 -->
		
		<c:set var="temp" value="임시 값" scope="session"/>
		<!-- session: 브라우저가 꺼질때까지 이 값을 사용할 수 있음 -->
		<h4>temp : ${temp}</h4>
		...
		<pre>
			...
			- c:remove 속성
				1)var : 삭제할 변수명(key)
				2)scope : 삭제할 내장객체 범위(기본값 : 모든범위)
				
			- session에 올라간 scope를 삭제하는 용도로 많이 이용
		</pre>
		...
		<pre>
			...
			* 주의사항 *
			1) test 속성 값 작성은 무조건 el구문이어야만 한다
		...
		<pre>
			c:choose  : when, otherwise를 감싸는 태그(현재태그 내부에 if ~ else if ~ else를 작성하겠다)
		...
		<pre>
			...
			- 속성
				...
				3) end : 반복이 종료되리 var 값
				4) step : 반복시마다 var의 증가값(기본값 1)
				...
				6) varStatus : 현재 반복 상태와 관련된 정보를 제공하는 변수 선언
					...
					* varStatus에서 제공되는 값 :
						- current : 현재 반복횟수(현재 var 값) 또는
							현재 반복접근중인 객체(컬렉션/배열의 요소)
						- index : 현재 몇 번째 인덱스 값 반환(0부터 시작)
	...
	<!-- 현재페이지 주소와 목표페이지 주소(forEach)가 같으므로 목표페이지 이름만 부르면 된다 -->
