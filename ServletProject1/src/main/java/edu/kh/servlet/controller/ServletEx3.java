...
//	doPost() : POST 방식 요청을 처리하는 메서드
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		// 왜? 브라우저, 서버간의 문자 인코딩 차이때문에. 
		// 해결방법 : 요청 정보에 담긴 데이터의 문자인코딩을 UTF-8로 변경한다
...
//		name속성값이 같은 input태그의 값은 String[] 하나로 얻어올 수 있다
		...
		out.println("<body>");
			if(inputPw[0].equals(inputPw[1])) { // 비밀번호가 같을 경우
				// user01님 환영 합니다.
				// 이메일 수신 동의 여부 : o / x
				// (동의 o인 경우) 수신할 이메일 : user01@gmail.com
				
				out.printf("<h3>%s님 환영합니다.</h3>", inputId);
				
//				checkbox는 별도의 value가 없을때 on(체크), null(체크x)
				out.printf("<h3>이메일 수신 동의 여부 : %s</h3>", agree != null? "O" : "X");
				if(agree != null) {
