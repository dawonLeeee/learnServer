 --관리자계정 들어오면 무조건 하는것(예전 쓰던 권한 그대로 쓰기)
ALTER SESSION SET "_ORACLE_SCRIPT" = TRUE;

 --수업용 프로젝트 계정 생성
CREATE USER project IDENTIFIED BY project1234;

 --권한 부여(접속관련, 객체생성, 뷰 생성 권한)
...
CREATE TABLE "MEMBER" (
	...
	"ENROLL_DATEL"	DATE	DEFAULT SYSDATE	NOT NULL,
...
COMMENT ON COLUMN "MEMBER"."MEMBER_EMAIL" IS '회원의 이메일(아이디로 사용)';
...
COMMENT ON COLUMN "MEMBER"."PROFILE_IMG" IS '프로필이미지 경로';

COMMENT ON COLUMN "MEMBER"."ENROLL_DATEL" IS '회원 가입일';
...
COMMENT ON COLUMN "BOARD"."BOARD_TITLE" IS '게세글 제목';
...
COMMENT ON COLUMN "BOARD_IMG"."IMG_NO" IS '이미지 번호(IMG_SEQ_NO)';

COMMENT ON COLUMN "BOARD_IMG"."IMG_PATH" IS '이미지 저장 폴더 경로';

COMMENT ON COLUMN "BOARD_IMG"."IMG_RENAME" IS '변경된 이미지파일 이름';

COMMENT ON COLUMN "BOARD_IMG"."IMG_ORIGINAL" IS '원본 이미지파일 이름';
...
COMMENT ON COLUMN "COMMENT"."COMMENT_DEL_FL" IS '삭제여부(N:삭제X, Y:삭제O)';

COMMENT ON COLUMN "COMMENT"."BOARD_NO" IS '댓글이 작성된 게시글번호';
